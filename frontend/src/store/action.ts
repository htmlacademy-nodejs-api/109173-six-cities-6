...
import { adaptCommentToServer, adaptOfferToServer, adaptUserToServer } from '../adapters/to-server.adapters';
...
export const fetchOffers = createAsyncThunk<Offer[], undefined, { extra: Extra }>(
  ...
  async (_, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<OffersListItemRDO[]>(ApiRoute.Offers);
...
export const fetchPremiumOffers = createAsyncThunk<Offer[], string, { extra: Extra }>(
  ...
  async (cityName, { extra }) => {
    const { api } = extra;
    const { data } = await api.get<OffersListItemRDO[]>(`${ApiRoute.Premium}/${cityName}`);
...
export const postComment = createAsyncThunk<Comment, CommentAuth, { extra: Extra }>(
  ...
  async ({ id, comment, rating }, { extra }) => {
    const adaptedComment = adaptCommentToServer({ id, comment, rating });
    const { api } = extra;
    const { data } = await api.post<Comment>(`${ApiRoute.Comments}`, adaptedComment);
